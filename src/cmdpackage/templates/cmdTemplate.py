...
syncTemplate = Template(
    dedent(
        """# Generated using classCall template
\"\"\"
syncTemps sync command - Synchronize files generated from templates
...
class TemplateSyncer:
        ...
        for field_key, placeholder_name in field_mappings:
            if field_key in fields and fields[field_key]:
                ...
                if field_value and field_value.strip():  # Only replace non-empty values
                    ...
                    if field_key == 'name':
                        # Handle compound patterns like "syncTempsrc", "syncTemplates", etc.
                        compound_patterns = [
    ...
    def _build_complete_commands_json_dict(self) -> str:
        ...
        commands_dir = os.path.join(self.project_root, 'src', 'syncTemps', 'commands')
    ...
    def _is_template_file_valid(self, template_file_path: str) -> bool:
        ...
        try:
            ...
            # Template files contain template variables like $packageName which aren't valid Python
    ...
    def sync_all_files(self, file_patterns: Optional[List[str]] = None) -> bool:
        ...
        if self.dry_run:
            ...
            if untracked_files:
                ...
                printIt(f"\\n  To track these files, use: {cStr('syncTemps sync make <file>', color.GREEN)}", lable.INFO)
...
class syncCommand:
    def __init__(self, argParse):
        ...
        # Get command-line flags from .syncTempsrc file after flag processing
        from ..classes.optSwitches import getCmdSwitchFlags
    ...
    def execute(self):
        ...
        if action == 'list':
            ...
            if not make_file:
                ...
                printIt("Usage: syncTemps sync make <filename>", lable.INFO)
                printIt("Example: syncTemps sync make tests/test_setLogPref_roundtrip.py", lable.INFO)
                return
            ...
            if not rmtemp_file:
                ...
                printIt("Usage: syncTemps sync rmTemp <filename>", lable.INFO)
                printIt("Example: syncTemps sync rmTemp tests/test_setLogPref_roundtrip.py", lable.INFO)
                return
