...
test_rmCmd_roundtrip_template = Template(
    dedent(
        """#!/usr/bin/env python3
\"\"\"
...
def check_sync_system_available() -> bool:
    ...
    returncode, stdout, stderr = run_command(
        "${packName} sync --help", capture_output=True
    )
    if returncode != 0:
        return False
...
def test_add_flags_and_arguments(result: TestResult) -> bool:
    ...
    all_ok = (
        verbose_ok
        and debug_ok
        and verbose_${packName}rc
        and debug_${packName}rc
        and arg1_ok
        and arg2_ok
    )
...
def test_remove_regular_argument(result: TestResult) -> bool:
    ...
    # Remove arg1
    returncode, stdout, stderr = run_command(
        'echo "y" | ${packName} rmCmd rmTestCmd01 arg1'
    )
...
def test_remove_switch_flag(result: TestResult) -> bool:
    ...
    returncode, stdout, stderr = run_command(
        'echo "y" | ${packName} rmCmd rmTestCmd01 verbose'
    )

    # Verify verbose flag was removed from all locations
    verbose_removed_json = not check_switch_flag_definition(
        "rmTestCmd01", "verbose", "bool"
    )
    verbose_removed_${packName}rc = not check_flag_in_${packName}rc(
        "rmTestCmd01", "verbose"
    )
...
def test_remove_remaining_items(result: TestResult) -> bool:
    ...
    returncode, stdout, stderr = run_command(
        'echo "y" | ${packName} rmCmd rmTestCmd01 arg2'
    )

    # Remove remaining flag
    returncode, stdout, stderr = run_command(
        'echo "y" | ${packName} rmCmd rmTestCmd01 debug'
    )

    # Verify everything was removed except basic command structure
    cmd_data = get_command_data("rmTestCmd01")
    arg2_removed = "arg2" not in cmd_data
    debug_removed_json = not check_switch_flag_definition(
