...
templateSources = {
    "test_rmCmd_roundtrip_template": "tests/test_rmCmd_roundtrip.py"
}
...
def check_in_commands_json_dict(cmd_name: str) -> bool:
    ...
    try:
        with open(template_path, 'r') as f:
            ...
            dict_end = content.find('\\ncommands', dict_start)  # Look for next section
            if dict_end == -1:
...
def test_template_system_cleanup(result: TestResult) -> bool:
    ...
    if returncode != 0:
        result.add_result("Template cleanup - create testVerify", False, f"Failed to create testVerify: {stderr}")
        return False
    ...
    if returncode != 0:
        result.add_result("Template cleanup - sync make", False, f"Failed to add to template system: {stderr}")
        return False
    ...
    if not sync_data_exists:
        result.add_result("Template cleanup - verify sync data", False, "testVerify not found in genTempSyncData.json")
        return False
    
    # Check testVerifyTemplate in cmdTemplate.py
    template_exists = check_template_in_cmdtemplate("testVerifyTemplate")
    if not template_exists:
        result.add_result("Template cleanup - verify template", False, "testVerifyTemplate not found in cmdTemplate.py")
        return False
    ...
    if not commands_dict_exists:
        result.add_result("Template cleanup - verify commands dict", False, "testVerify not found in commandsJsonDict")
        return False
    
    result.add_result("Template cleanup - verification", True, "testVerify found in all expected locations")
    ...
    if returncode != 0:
        result.add_result("Template cleanup - remove command", False, f"Failed to remove testVerify: {stderr}")
        return False
    ...
    if not sync_data_removed:
        result.add_result("Template cleanup - sync data removal", False, "testVerify still found in genTempSyncData.json")
        return False
    ...
    if not template_removed:
        result.add_result("Template cleanup - template removal", False, "testVerifyTemplate still found in cmdTemplate.py")
        return False
    ...
    if not commands_dict_removed:
        result.add_result("Template cleanup - commands dict removal", False, "testVerify still found in commandsJsonDict")
        return False
    ...
    if not command_removed:
        result.add_result("Template cleanup - commands.json removal", False, "testVerify still found in commands.json")
        return False
    
    result.add_result("Template cleanup - complete removal", True, "testVerify successfully removed from all locations")
    return True
