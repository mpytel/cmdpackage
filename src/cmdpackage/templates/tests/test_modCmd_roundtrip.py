...
def test_add_boolean_flags(result: TestResult) -> bool:
    ...
    force_flag_ok = (
        switch_flags.get("f", {}).get("type") == "bool"
        and switch_flags.get("f", {}).get("description") == "Force operation flag"
    )
    quiet_flag_ok = (
        switch_flags.get("q", {}).get("type") == "bool"
...
def test_add_string_options(result: TestResult) -> bool:
    ...
    input_option_ok = (
        switch_flags.get("input", {}).get("type") == "str"
        and switch_flags.get("input", {}).get("description") == "Input file path"
    )
    loglevel_option_ok = (
        switch_flags.get("loglevel", {}).get("type") == "str"
...
def test_modify_mixed_args_and_flags(result: TestResult) -> bool:
    ...
    switch_flags = cmd_data.get("switchFlags", {})
    flag_ok = (
        switch_flags.get("e", {}).get("type") == "bool"
        and switch_flags.get("e", {}).get("description") == "Enable feature flag"
    )
    option_ok = (
        switch_flags.get("format", {}).get("type") == "str"
...
def test_modify_existing_flags(result: TestResult) -> bool:
    ...
    verbose_ok = (
        switch_flags.get("v", {}).get("description")
        == "Updated verbose flag description"
    )
    debug_ok = (
        switch_flags.get("d", {}).get("description") == "Updated debug flag description"
    )
