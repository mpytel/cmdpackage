...
templateSources = {
    "test_modCmd_roundtrip_template": "tests/test_modCmd_roundtrip.py",
}
...
def cleanup_test_commands():
    ...
    test_commands = ["modTestCmd01", "modTestCmd02", "modTestCmd03", "modTestFlags", "modTestString"]
...
def setup_test_commands():
    ...
    returncode, stdout, stderr = run_command("${packName} newCmd modTestCmd01 arg1 arg2", input_text)
    ...
    returncode, stdout, stderr = run_command("${packName} newCmd modTestCmd02 arg1 -v -d", input_text)
    ...
    input_text = "Test command with string options\\nOutput file option\\nConfig file option"
    returncode, stdout, stderr = run_command("${packName} newCmd modTestCmd03 --output --config", input_text)
...
def test_modify_command_description(result: TestResult) -> bool:
    ...
    if cmd_data.get('description') == "Updated description for test command":
        ...
        result.add_result("Modify command description", False, "Description not updated in commands.json")
        return False
...
def test_modify_existing_argument(result: TestResult) -> bool:
    ...
    if cmd_data.get('arg1') == "Updated description for first argument":
        ...
        result.add_result("Modify existing argument", False, "Argument description not updated")
        return False
...
def test_add_new_argument(result: TestResult) -> bool:
    ...
    if cmd_data.get('arg3') == "New third argument description":
        ...
        result.add_result("Add new argument", False, "New argument not found in commands.json")
        return False
...
def test_add_boolean_flags(result: TestResult) -> bool:
    ...
    force_flag_ok = (switch_flags.get('f', {}).get('type') == 'bool' and 
                     switch_flags.get('f', {}).get('description') == 'Force operation flag')
    quiet_flag_ok = (switch_flags.get('q', {}).get('type') == 'bool' and 
                     switch_flags.get('q', {}).get('description') == 'Quiet mode flag')
...
def test_add_string_options(result: TestResult) -> bool:
    ...
    returncode, stdout, stderr = run_command("${packName} modCmd modTestCmd01 --input --loglevel", input_text)

    # Check if options were added to commands.json
    cmd_data = get_command_data("modTestCmd01")
    switch_flags = cmd_data.get('switchFlags', {})
    
    input_option_ok = (switch_flags.get('input', {}).get('type') == 'str' and 
                       switch_flags.get('input', {}).get('description') == 'Input file path')
    loglevel_option_ok = (switch_flags.get('loglevel', {}).get('type') == 'str' and 
                          switch_flags.get('loglevel', {}).get('description') == 'Log level setting')
    ...
    if input_option_ok and loglevel_option_ok and input_${packName}rc_ok and loglevel_${packName}rc_ok:
        ...
        result.add_result("Add string options", False, "Options not properly configured")
        return False
...
def test_modify_mixed_args_and_flags(result: TestResult) -> bool:
    ...
    returncode, stdout, stderr = run_command("${packName} modCmd modTestCmd01 -e --format", input_text)
    ...
    # Check switch flags
    switch_flags = cmd_data.get('switchFlags', {})
    flag_ok = (switch_flags.get('e', {}).get('type') == 'bool' and 
               switch_flags.get('e', {}).get('description') == 'Enable feature flag')
    option_ok = (switch_flags.get('format', {}).get('type') == 'str' and 
                 switch_flags.get('format', {}).get('description') == 'Output format option')
    ...
    if desc_ok and arg_ok and flag_ok and option_ok and flag_${packName}rc_ok and option_${packName}rc_ok:
        ...
        print_info(f"Description OK: {desc_ok}, Arg OK: {arg_ok}, Flag OK: {flag_ok}, Option OK: {option_ok}")
        print_info(f"Flag ${packName}RC OK: {flag_${packName}rc_ok}, Option ${packName}RC OK: {option_${packName}rc_ok}")
        result.add_result("Modify mixed args and flags", False, "Not all modifications applied")
        return False
...
def test_modify_nonexistent_command(result: TestResult) -> bool:
    ...
    if "does not exists" in stdout or "does not exist" in stdout:
        ...
        result.add_result("Modify non-existent command", False, "Should report command doesn't exist")
        return False
...
def test_modify_no_changes(result: TestResult) -> bool:
    ...
    if original_data == new_data:
        ...
        if "modified" in stdout:
            print_pass("No changes handling - command reports modified (expected behavior)")
            result.add_result("Modify with no changes", True)
            return True
        else:
            print_fail("No changes scenario not handled properly")
            result.add_result("Modify with no changes", False, "Unexpected behavior when declining changes")
            return False
...
def test_modify_existing_flags(result: TestResult) -> bool:
    ...
    verbose_ok = switch_flags.get('v', {}).get('description') == 'Updated verbose flag description'
    debug_ok = switch_flags.get('d', {}).get('description') == 'Updated debug flag description'
    ...
    if verbose_ok and debug_ok:
        ...
        result.add_result("Modify existing flags", False, "Flag descriptions not updated")
        return False
...
def test_modify_with_empty_descriptions(result: TestResult) -> bool:
    ...
    input_text = "\\n\\n"  # Empty descriptions
    returncode, stdout, stderr = run_command("${packName} modCmd modTestEmpty arg1 -v", input_text)
    ...
    flag_desc = switch_flags.get('v', {}).get('description', '')
    ...
    if has_default:
        ...
        result.add_result("Empty descriptions", False, "No default description provided")
        return False
...
def test_invalid_flag_formats(result: TestResult) -> bool:
    ...
    # Set up a test command first
    input_text = "Test command\\nTest arg"
    returncode, stdout, stderr = run_command("${packName} newCmd modTestInvalid arg1", input_text)
    ...
    invalid_handled = ("Missing option name" in stdout or 
                      "Missing option name" in stderr or
                      returncode != 0)
    ...
    invalid_handled2 = ("Missing option name" in stdout2 or 
                       "Missing option name" in stderr2 or
                       returncode2 != 0)
    ...
    if invalid_handled and invalid_handled2:
        ...
        result.add_result("Invalid flag formats", False, "Should handle invalid flag formats")
        return False
